...
class LatinQuizApp:
    def __init__(self, root):
        ...
        self.timer_font = font.Font(family="Arial", size=20, weight="bold")
    ...
    def show_question(self):
        ...
        # Timer display
        self.timer_label = tk.Label(self.root, text=f"Time: {self.time_remaining}s", 
                                    font=self.timer_font, bg="#f0f0f0", fg="#e74c3c")
    ...
    def update_timer(self):
        ...
        if self.time_remaining > 0 and not self.answered:
            self.timer_label.config(text=f"Time: {self.time_remaining}s")
            
            # Change color based on time remaining
            if self.time_remaining <= 5:
                self.timer_label.config(fg="#e74c3c")  # Red
            elif self.time_remaining <= 10:
                self.timer_label.config(fg="#f39c12")  # Orange
            else:
                self.timer_label.config(fg="#27ae60")  # Green
            
            self.time_remaining -= 1
            self.timer_id = self.root.after(1000, self.update_timer)
        elif self.time_remaining == 0 and not self.answered:
            # Time's up!
            self.time_up()
    ...
    def time_up(self):
        ...
        self.show_explanation(False, "No answer (time expired)")
